                        ...
                        <div class="portlet light portlet-fit portlet-datatable bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    ...
                                    <span class="caption-subject font-dark sbold uppercase">该服务器中各接口运行状况</span>
                                </div>
                            </div>
                            <div class="portlet-body">
                                <div class="table-container">
...
<script>
    ...
    $(function(){
        var loadPage = function(){
            ...
            var id = App.getURLParameter("server");
            if(id){
                showDetails(project,timespan,id);
                $('#allDetails').hide();
                $('#headerInfo').html("<h3>您正在查看的服务器ID为"+id+",名称为"+App.getURLParameter("name")+"</h3>").show();
            } else {
                loadServer(project, timespan);
            }
        };
        //绑定toggle点击事件。
...
//            }],
        ...
        var table_port;
        var showDetails = function (project, timespan, server_id) {
            ...
            table_port = $('#table_port').DataTable($.extend({
                "ajax": {
                    "url": "/getServerIFInfo",
                    "data": function ( d ) {
                        return $.extend( {}, d, {
            ...
            $.get('/getServerAffairsInfo',{"id_project":project,"timespan":timespan,"id_server":server_id},function(data){
                var option = {
            ...
            setTimeout(function(){
                $.get('/getServerMap',{"id_project":project,"timespan":timespan,"id_server":server_id},function(data){
        ...
        var table_server;
        function loadServer(project,timespan) {
            //所有服务器
            // begin first table
            if ($.fn.dataTable.isDataTable('#server_table')) {
                table_server.destroy();
            }
            table_server= $('#server_table').DataTable($.extend({
                "ajax": {
                    "url": "/getServerInfo",
                    "data": function (d) {
                        return $.extend({}, d, {
            ...
            table_server.off('click').on('click', 'tr', function () {
                var self = $(this);
                if (self.hasClass('selected') || self.hasClass('heading')) {
                    return;
                }
                else {
                    table_server.$('tr.selected').removeClass('selected');
                    self.addClass('selected');
                    var server_id = self.find('td:first').text();
                    showDetails(project, timespan, server_id);
                }
            });
        }
        loadPage();
