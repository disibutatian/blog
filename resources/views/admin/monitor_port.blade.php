                    ...
                    <div class="portlet light portlet-fit portlet-datatable bordered" id="allDetails">
                        ...
                        <div class="portlet-body">
                            <div class="table-container">
                                <table class="table table-striped table-bordered table-hover table-checkable" id="port_table">
                                    <thead>
                    ...
                    <div class="portlet light portlet-fit portlet-datatable bordered">
                        <div class="portlet-title">
                            <div class="caption">
                                ...
                                <span class="caption-subject font-dark sbold uppercase">该接口在各服务器上运行状况</span>
                            </div>
                        </div>
                        <div class="portlet-body">
                            <div class="table-container">
...
<script>
    ...
    $(function(){
        var loadPage = function(){
            ...
            var id = App.getURLParameter("port");
            if(id){
                showDetails(project,timespan,id);
                $('#allDetails').hide();
                $('#headerInfo').html("<h3>您正在查看的接口ID为"+id+",名称为"+App.getURLParameter("name")+"</h3>").show();
            } else {
                loadPort(project, timespan);
            }
        };
        //绑定toggle点击事件。
...
//            }],
        ...
        var table_server;
        var showDetails = function (project,timespan,port_id) {
            ...
            $.get('/getIFAffairsInfo',{"id_project":project,"timespan":timespan,"id_IF":port_id},function(data){
                var option = {
            ...
            setTimeout(function(){
                $.get('/getIFMap',{"id_project":project,"timespan":timespan,"id_IF":port_id},function(data){
        ...
        var table_port;
        function loadPort(project,timespan){
            //所有接口
            // begin first table
            if ($.fn.dataTable.isDataTable('#port_table')) {
                table_port.destroy();
            }
            table_port= $('#port_table').DataTable( $.extend({
                "ajax": {
            ...
            table_port.off('click').on( 'click', 'tr', function () {
                var self = $(this);
                if ( self.hasClass('selected') || self.hasClass('heading') ) {
                    return;
                }
                else {
                    table_port.$('tr.selected').removeClass('selected');
                    self.addClass('selected');
                    var port_id  =self.find('td:first').text();
                    showDetails(project,timespan,port_id);
                }
            } );
        }
